...
class NewsCategories extends Model
{
    ...
    private static $translates_class = \App\Models\NewsCategoriesTranslate::class;

    private static $main_table = 'news_categories';

    private static $translates_table = 'news_categories_translates';
    ...
    {
        return $this->hasOne(NewsCategoriesTranslate::class, 'parent_id', 'id')->where('lang', locale());
    }

    public function blogs(): HasMany
    {
        return $this->hasMany(\App\Models\News::class, 'category_id')->where('status', 1);
    }
            ...
            if ($status_on) {
                return DB::select("
                        SELECT
                            news_categories.*,
                            nct.title,
                            IFNULL(B.newses,0) AS newses
                        FROM news_categories
                        LEFT JOIN
                        (SELECT category_id, COUNT(id) AS newses FROM news WHERE status=1 GROUP BY category_id) AS B
                            ON news_categories.id=B.category_id
                        LEFT JOIN news_categories_translates AS nct
                            ON news_categories.id=nct.parent_id
                        WHERE news_categories.status=1 AND nct.lang='" . $local . "';
                        ");
            } else {
                return DB::select("
                        SELECT
